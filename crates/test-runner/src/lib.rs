...
pub fn test_snapshots(
    ...
    if let Err(TimeoutError::Timeout) = signal_end.wait_timeout_ms(TIMEOUT) {
        bail!("Timed out");
    }
...
fn test_snapshots_inner(
    ...
    let failed = processed - successes;

    eprintln!("\nProcessed {section_name}: {processed}, Failed: {failed}, Skipped: {skipped}",);
